...
int main(void)
{
    ...
    // initializing the player's color
    if(compColour == 'B') {
        playerColour = 'W';
    ...
    // printing the initial board or the board after the first computer's move
    printBoard(board, dimension);
    
    // a while loop to go through the players move and then the AI's move in turn when both players have moves available
    do {
        if(areMovesAvailable(board, dimension, playerColour)) {
            ...
            // informing the player if they don't have a valid move
            printf("%c player has no valid move.\n", playerColour);
        }
        
        // the AI plays a move if it has one available
        if(areMovesAvailable(board, dimension, compColour)) {
            makeMoveAI(board, dimension, compColour);
            printBoard(board, dimension);
        } else if(areMovesAvailable(board, dimension, playerColour)) {
            // if none available, printing a message to let the player know
            printf("%c player has no valid move.\n", compColour);
        }
    } while(areMovesAvailable(board, dimension, playerColour) || areMovesAvailable(board, dimension, compColour));
    
    // when the game is done, this function lets the player know who won
...
// the function which checks if a colour has any moves available
bool areMovesAvailable(char board[ ][26], int n, char colour)
{
    // declaring local variables
    int rowCounter, colCounter;
    
    // going through eacch cell on the board to check if that's a valid move
    for(rowCounter = 0; rowCounter < n; rowCounter++) {
        for(colCounter = 0; colCounter < n; colCounter++) {
...
// the function which flips tiles in a certain direction only
void flipTilesInDirection(char board[ ][26], int n, char row, char col, char colour, int deltaRow, int deltaCol)
{
    // declaring local variables 
    int rowInt = ((int) row) - 97;
...
// the function which scores a move in a certain direction only
int scoreMoveInDirection(char board[ ][26], int n, char row, char col, char colour, int deltaRow, int deltaCol)
{
    // declaring local variables 
    int rowInt = ((int) row) - 97;
...
int scoreMove(char board[ ][26], int n, char row, char col, char colour)
{
    // the score for a move is the score for each direction for that move all added up
    int score;
...
// the function whih computes and informs who won
void didPlayerWin(char board[ ][26], int n, char playerColour)
{
    // declaring and initializing local variables
    ...
    // computing scores for each colour
    ...
    // deciding who won and printing that out
